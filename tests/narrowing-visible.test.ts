...
function scoreLine(line: string, goalTokens: string[]): number {
...
function topLines(lines: string[], goalTokens: string[], keep: number): string[] {
  return lines
...
function extractChunkLines(chunks: string[]): string[] {
...
function recursiveSelectWithTrajectory(
  chunks: string[],
  goalTokens: string[],
  depth: number,
  maxDepth: number,
  ...
  const left = recursiveSelectWithTrajectory(
    chunks.slice(0, mid),
    goalTokens,
    depth + 1,
    maxDepth,
  ...
  const right = recursiveSelectWithTrajectory(
    chunks.slice(mid),
    goalTokens,
    depth + 1,
    maxDepth,
...
function mockPaperStyleRLMWithTrajectory(
  archiveContext: string,
  latestGoal: string,
  config: RecursiveConfig,
): { preparedContext: string; trajectory: TrajectoryNode } {
  const chunks = archiveContext.split(/\n\n+/).filter((chunk) => chunk.trim().length > 0)
  const goalTokens = tokenize(latestGoal)
  const selection = recursiveSelectWithTrajectory(
    chunks,
    goalTokens,
    0,
    config.maxDepth,
...
function flattenTrajectory(root: TrajectoryNode): TrajectoryNode[] {
  const nodes: TrajectoryNode[] = [root]
...
function printTrajectory(root: TrajectoryNode): void {
  console.log("TRAJECTORY TREE")

  const visit = (node: TrajectoryNode, indent: string): void => {
    const preview = node.selected.slice(0, 2).join(" | ") || "(none)"
    console.log(
      `${indent}#${node.id} depth=${node.depth} phase=${node.phase} chunks=${node.chunkCount} iterations_left=${node.iterationsLeft} selected=${node.selected.length}`,
    )
...
function printSideBySide(noisy: string, prepared: string): void {
  const width = 58
...
function buildRealisticMessages(): ChatMessage[] {
  return [
    textMessage(
      "user",
      "I copied a long terminal dump with npm warnings, lockfile churn, and shell history that is not tied to the current fix.",
    ),
    textMessage(
      "assistant",
      "We can redesign the landing hero gradient, typography stack, and button color palette later.",
    ),
    textMessage(
      "assistant",
      "Keep backend default to opencode in lib/config.ts so session auth is used without external provider keys.",
    ),
    textMessage(
      "assistant",
      "In lib/opencode-bridge.ts preserve session cleanup in finally and keep temporary session deletion resilient.",
    ),
    textMessage(
      "assistant",
      "tests/opencode-bridge.test.ts should verify model override is optional and defaults to active session model.",
    ),
    textMessage(
      "user",
      "Finalize opencode-auth migration and make sure the plugin no longer depends on OPENAI_API_KEY for the default path.",
    ),
  ]
}
...
test("paper-style recursive narrowing keeps constraints and purges unrelated chatter", async () => {
  ...
  let noisyArchive = ""
  let preparedContext = ""
  let trajectory: TrajectoryNode | null = null
...
test("paper-style recursive narrowing exposes trajectory tree nodes", async () => {
  ...
  assert.ok(
    nodes.some((node) =>
      node.selected.some((line) => /opencode|cleanup|openai_api_key|session model/i.test(line)),
    ),
  )
  assert.equal(/gradient|typography|color palette/.test(preparedContext.toLowerCase()), false)
