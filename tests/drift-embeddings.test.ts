...
test("detectContextDriftWithEmbeddings detects drift with /api/embed response", async () => {
  const fetchMock = async (_url: string, _init?: unknown): Promise<MockResponse> => {
    return jsonResponse({
      embeddings: [
        [1, 0],
        [0, 1],
  ...
  const drift = await detectContextDriftWithEmbeddings(
    "archive mentions migration and opencode backend",
    "recent discusses unrelated ui color polishing",
    "finalize opencode auth migration",
    { ...BASE_CONFIG, driftThreshold: 0.45 },
...
test("detectContextDriftWithEmbeddings falls back to /api/embeddings when /api/embed fails", async () => {
  let embedCalls = 0
  let legacyCalls = 0
  const fetchMock = async (url: string, _init?: unknown): Promise<MockResponse> => {
  ...
  const drift = await detectContextDriftWithEmbeddings(
    "archive keeps backend migration constraints",
    "recent asks for unrelated css updates",
    "finish backend migration",
    { ...BASE_CONFIG, driftThreshold: 0.4 },
    fetchMock as unknown as typeof fetch,
  )
...
test("detectContextDriftWithEmbeddings returns no-drift when disabled or provider is unsupported", async () => {
  ...
  const disabled = await detectContextDriftWithEmbeddings(
    "archive",
    "recent",
    "goal",
    { ...BASE_CONFIG, driftEmbeddingsEnabled: false },
    fetchMock as unknown as typeof fetch,
  )
  const unsupported = await detectContextDriftWithEmbeddings(
    "archive",
    "recent",
    "goal",
    { ...BASE_CONFIG, driftEmbeddingProvider: "none" },
    fetchMock as unknown as typeof fetch,
  )
