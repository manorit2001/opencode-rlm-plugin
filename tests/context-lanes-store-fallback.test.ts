...
test("ContextLaneStore works with in-memory fallback when node:sqlite is disabled", () => {
  ...
  try {
    ...
    store.appendProgressionStep(
      ...
      "msg-1",
      "routing.completed",
      JSON.stringify({ primaryContextID: created.id }),
      now + 4_500,
    )
    ...
    store.saveContextSnapshot(
      ...
      "model-input",
      0,
      JSON.stringify({ historyMessages: 3 }),
      now + 5_000,
    )
...
test("ContextLaneStore persists data when sqlite backend is available", { skip: !hasNodeSqlite() }, () => {
  ...
  try {
    ...
    reopened.saveContextSnapshot(
      "session-sqlite",
      "msg-sqlite",
      "model-input",
      0,
      JSON.stringify({ historyMessages: 2 }),
      now + 3_000,
    )
    reopened.appendLaneEvent(
      "session-sqlite",
      "msg-sqlite",
      "context.prepared",
      JSON.stringify({ historyMessages: 2 }),
