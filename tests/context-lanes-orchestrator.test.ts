...
function textMessage(id: string, role: string, text: string): ChatMessage {
  return {
    id,
    role,
...
test("orchestrator supports multi-lane relevance and lane-scoped history", async () => {
  await withStore(async (store, orchestrator) => {
    ...
    const backend = store.createContext(
      "session-b",
      "Backend Migration",
      "- keep opencode backend default and preserve cleanup in finally",
      now - 60_000,
    )
    const tests = store.createContext(
      "session-b",
      "Regression Tests",
      "- backend migration tests and bridge checks for keyless mode",
      now - 55_000,
    )
    ...
    const laneHistoryIDs = new Set(
      routed.laneHistory.map((message) => message.id).filter((id): id is string => typeof id === "string"),
    )
...
test("orchestrator tracks active lane switches and emits switch events", async () => {
  await withStore(async (store, orchestrator) => {
    ...
    const tests = store.createContext(
      "session-c",
      "Regression Tests",
      "- update bridge tests and migration checks for keyless mode",
      now - 110_000,
    )
...
test("manual override pins primary lane until expiry", async () => {
  await withStore(async (store, orchestrator) => {
    ...
    const backend = store.createContext(
      "session-d",
      "Backend Migration",
      "- keep backend default and cleanup",
      now - 60_000,
    )
    const tests = store.createContext(
      "session-d",
      "Regression Tests",
      "- update bridge tests and migration checks",
      now - 55_000,
    )
...
test("orchestrator falls back to full history when lane-specific history is too small", async () => {
  await withStore(async (store, orchestrator) => {
    ...
    store.saveMemberships(
      "session-e",
      "e4",
      [{ contextID: backend.id, relevance: 0.8, isPrimary: true }],
      now - 5_000,
    )
