...
function parseEvents(output: string): OpencodeEvent[] {
...
function extractSessionID(events: OpencodeEvent[]): string {
...
function extractToolOutput(events: OpencodeEvent[], toolName: string): string {
...
function runTurn(prompt: string, env: NodeJS.ProcessEnv, sessionID?: string): RunTurnResult {
...
function parseContextsOutput(output: string): { primaryContextID: string | null; contextIDs: string[] } {
...
const liveSkip =
  process.env[LIVE_TEST_FLAG] === "1"
    ? false
    : `Set ${LIVE_TEST_FLAG}=1 to run live lane-switch CLI integration test.`

test(
  "Live OpenCode session records manual lane switch using gpt-5.1-codex-mini",
  {
    skip: liveSkip,
    timeout: 600_000,
  },
  () => {
    ...
    try {
      ...
      const switchTurn = runTurn(
        `Use contexts-switch with contextID='${switchTarget}' and ttlMinutes=10, then reply with exactly: switch done`,
        env,
        sessionID,
      )
