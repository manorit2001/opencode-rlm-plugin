...
import { mergeSemanticScores, scoreContextsForMessage, selectContextLanes, shouldRunSemanticRerank, } from "../lib/context-lanes/router.js";
const BASE_CONFIG = {
    enabled: true,
...
function lane(id, title, summary, now) {
    return {
        id,
        sessionID: "session-1",
        title,
...
test("router scores all contexts and supports primary+secondary selection", () => {
    const now = Date.now();
    const contexts = [
        lane("backend", "Backend Auth Migration", "Keep opencode backend default and preserve bridge cleanup in finally.", now),
    ...
    const scores = scoreContextsForMessage("Finalize backend auth migration and update bridge tests for keyless opencode mode.", contexts, now);
...
test("router returns no primary when all scores are below primary threshold", () => {
    const selected = selectContextLanes([{ contextID: "x", score: 0.1, title: "x" }], null, { ...BASE_CONFIG, lanePrimaryThreshold: 0.2 });
...
test("router prefers stronger lexical match over fresher but weak overlap", () => {
    const now = Date.now();
    const contexts = [
        lane("recent-generic", "Recent General", "General planning updates and daily notes", now),
        lane("older-precise", "Bridge Cleanup", "Preserve bridge cleanup in finally for keyless opencode migration", now - 45 * 60_000),
    ];
    const scores = scoreContextsForMessage("Ensure keyless opencode migration keeps bridge cleanup in finally.", contexts, now);
    const selected = selectContextLanes(scores, null, BASE_CONFIG);
...
test("router merges semantic similarity as bounded boost", () => {
    ...
    const merged = mergeSemanticScores(lexicalScores, new Map([
        ["backend", 0.05],
        ["tests", 0.95],
