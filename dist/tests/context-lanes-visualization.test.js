...
import { buildLaneVisualizationSnapshot, formatLaneVisualizationText, renderLaneVisualizationHTML, } from "../lib/context-lanes/visualization.js";
test("buildLaneVisualizationSnapshot returns per-session lane timelines", () => {
    ...
    try {
        ...
        const planning = store.createContext("session-a", "Planning", "Plan lane summary", now, "context-plan");
        const execution = store.createContext("session-a", "Execution", "Execution lane summary", now + 200, "context-exec");
        store.saveMemberships("session-a", "msg-1", [
            {
        ...
        store.saveMemberships("session-a", "msg-2", [
            {
        ...
        store.recordSwitch("session-a", "msg-2", planning.id, execution.id, 0.92, "score-switch", now + 2_000);
