...
test("orchestrator can create session-backed lane ids and prefixed titles", async () => {
    const dir = mkdtempSync(join(tmpdir(), "rlm-lanes-"));
    const store = new ContextLaneStore(dir, "lane-state.sqlite");
    const orchestrator = new ContextLaneOrchestrator(store, fetch, async ({ rootSessionID, laneTitle }) => ({
        contextID: `child-${rootSessionID}`,
        laneTitle: `Project: ${laneTitle}`,
    }));
    try {
        const now = Date.now();
        const history = [textMessage("sb1", "user", "backend migration plan")];
        const routed = await orchestrator.route({
...
test("orchestrator keeps routed lane history in strict arrival order", async () => {
    await withStore(async (store, orchestrator) => {
        const now = Date.now();
        const backend = store.createContext("session-order", "Backend Migration", "- backend migration plan and rollout checklist", now - 80_000, "backend-order");
        const tests = store.createContext("session-order", "Regression Tests", "- regression tests and migration validation", now - 75_000, "tests-order");
        store.saveMemberships("session-order", "o1", [{ contextID: backend.id, relevance: 0.9, isPrimary: true }], now - 70_000);
        store.saveMemberships("session-order", "o2", [{ contextID: tests.id, relevance: 0.9, isPrimary: true }], now - 65_000);
        store.saveMemberships("session-order", "o3", [{ contextID: backend.id, relevance: 0.85, isPrimary: true }], now - 60_000);
        const history = [
...
test("orchestrator produces deterministic lane routing traces for replayed turn order", async () => {
    async function runTrace() {
        ...
        await withStore(async (store, orchestrator) => {
            const now = 500_000;
            store.createContext("session-replay", "Backend Migration", "- backend migration work queue and checkpoint tracking", now - 90_000, "backend-replay");
            store.createContext("session-replay", "Regression Tests", "- regression tests, coverage, and migration verification", now - 90_000, "tests-replay");
            const turns = [
...
test("orchestrator keeps loop-like routing in one lane and bounded by full-history tokens", async () => {
    await withStore(async (store, orchestrator) => {
        const now = Date.now();
        const backend = store.createContext("session-loop", "Backend Migration", "- backend migration loop checkpoint queue and rollout stability", now - 50_000, "backend-loop");
        const history = [
            textMessage("l0", "user", "Continue backend migration rollout and validation."),
        ];
        store.saveMemberships("session-loop", "l0", [{ contextID: backend.id, relevance: 0.95, isPrimary: true }], now - 45_000);
