...
function inferLastActivityAt(context, switches, memberships, fallback) {
    let max = fallback;
...
function timelineFrom(contexts, memberships, switches) {
    const timeline = [];
...
function selectedSessions(sessions, requestedSessionID, sessionLimit) {
...
export function buildLaneVisualizationSnapshot(laneStore, laneOrchestrator, options = {}) {
    ...
    const visualizedSessions = sessions.map((session) => {
...
export function formatLaneVisualizationText(snapshot) {
    ...
    for (const session of snapshot.sessions) {
        lines.push(`Session ${session.sessionID} | primary=${session.primaryContextID ?? "none"} | active=${session.activeContextCount} | contexts=${session.contexts.length} | memberships=${session.memberships.length} | switches=${session.switches.length}`);
    }
    return lines.join("\n");
}
export function renderLaneVisualizationHTML(snapshot, options = {}) {
